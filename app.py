...
def index():
    if (db.wellness.count_documents({}) > 0):
        ...
        admin_emails = [user['email'] for user in db.Users.find_one({}, sort=[( '_id', -1 )])]
...
def get_form_submission():
    ...
    for col in columns_to_process:
        if col in data:
            data[col] = float(data[col]) if data[col] != '' else 4.0
    emotional_sum = sum(data.get(f'em{i}', 4.0) for i in range(1, 11))
    occupational_sum = sum(data.get(f'oc{i}', 4.0) for i in range(1, 11))
    spiritual_sum = sum(data.get(f'sp{i}', 4.0) for i in range(1, 11))
    physical_sum = sum(data.get(f'ph{i}', 4.0) for i in range(1, 11))
    social_sum = sum(data.get(f'so{i}', 4.0) for i in range(1, 11))
    financial_sum = sum(data.get(f'fi{i}', 4.0) for i in range(1, 11))
    intellectual_sum = sum(data.get(f'in{i}', 4.0) for i in range(1, 11))
    environmental_sum = sum(data.get(f'en{i}', 4.0) for i in range(1, 11))
    ...
    data['dimension'] = min(dimensions, key=dimensions.get)
    db.wellness.insert_one(data)
    recipient_email = data.get('email')
    msg = Message('Wellness Cup assessment: New Submission Receieved!', recipients=[recipient_email])
